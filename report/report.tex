...
$$\dot{\vec{C}}_n = -\frac{i}{\hbar}\sum_{k,m=1}^N\mathbbm{S}^{-1}_{nm}\left(\mathbbm{H}_{mk}-i\hbar\boldsymbol{\tau}_{mk}\right)\vec{C}_k\text{ --- vector of coeficients}$$
...
\dot{\mathbbm{H}}_{mk}=2\mathit{Re}\sum_{l=1}^M\dot{\lambda}_{kl}\mathbbm{H}_{mk}^{(l0)}$$
$$\frac{\mymean{\Psi}{\hat{H}}{\Psi}}{\myel{\Psi}{\Psi}}=\vec{C}^{\dagger} \cdot 2\mathit{Re}(\mathbbm{Y}\dot{\Lambda})\cdot\vec{C},\ %
